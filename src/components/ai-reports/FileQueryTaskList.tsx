...
interface FileQueryTaskListProps {
...
export function FileQueryTaskList({ onViewTask }: FileQueryTaskListProps) {
  ...
  const loadTasks = useCallback(async (page: number = 0) => {
    ...
    try {
      ...
      const response = await ServiceRegistry.fileQueryBackground.getUserTasks(
        user.user_id, 
        limit
      );
      ...
      setError('Failed to load file query tasks. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [user?.user_id, limit]);
  ...
  const handleViewTask = (task: BackgroundTask) => {
    router.push(`/history/file-query/${task.task_id}`);
  };
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      const response = await ServiceRegistry.fileQueryBackground.deleteTask(taskToDelete);
      ...
      <Card className="bg-red-900/20 border-red-500/30">
        <CardContent className="p-8 text-center">
          ...
          <h3 className="text-white text-lg font-medium mb-2">
            Error Loading File Query Tasks
          </h3>
        ...
        <div className="modal-content-enhanced ai-reports-full-height">
          <div className="px-6 py-12 text-center">
            <div className="flex flex-col items-center justify-center h-32">
              <span className="text-white/70 text-lg font-medium">No Completed File Queries</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="modal-enhanced">
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={3} className="px-3 sm:px-6 py-8 text-center">
                        <span className="text-white/70 text-base font-medium">No completed file queries found</span>
                      </td>
                    </tr>
                  ) : (
                        ...
                        <td className="px-3 sm:px-6 py-4">
                          <div className="text-white font-medium">
                            <div className="truncate max-w-[200px] sm:max-w-none">
                              {task.query ? task.query.substring(0, 50) + (task.query.length > 50 ? '...' : '') : 'File Query'}
                            </div>
                          </div>
                        </td>
      ...
      <ConfirmationDialog
        ...
        title="Delete File Query Task"
        message="Are you sure you want to delete this file query task? This action cannot be undone and will permanently remove the task from your history."
        confirmText="Delete"
        cancelText="Cancel"
        variant="destructive"
